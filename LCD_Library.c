...
void lcdInit() {
    ...
    lcdWriteCmd(CLEAR);         delay_ms(100);  //clear screen
    ...
    //-------------------------------------------------------------------------------------
    lcdWriteCmd(FSET_100);      delay_ms(100);  //start reset
    lcdWriteCmd(FSET_100);      delay_ms(100);  //reinforce reset
    lcdWriteCmd(FSET_100);      delay_ms(100);  //reset captured
    lcdWriteCmd(FSET_100);      delay_ms(100);  //send real "function set" call (same as others but it is still needed)

    // HOME command is 0x02
    lcdWriteCmd(HOME);          delay_ms(100);  //send cursor home
    ...
    //-------------------------------------------------------------------------------------
    lcdWriteCmd(ENTRYMODE_10);  delay_ms(100);  //set up for auto incrementing

    lcdWriteCmd(CLEAR);         delay_ms(100);  //clear screen (again)
    ...
    //-------------------------------------------------------------------------------------
    lcdWriteCmd(DISPLAY_111);   delay_ms(100);  //turn on display with blinking cursor
...
/// ****| lcdStoreCustomChar | **********************//*
/// * Brief: Stores a custom character into CGRAM based on
